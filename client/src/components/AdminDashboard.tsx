...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ currentUser }) => {
  ...
  const [adminActiveTab, setAdminActiveTab] = useState<"accounts" | "assignments" | "directory">("accounts");
  ...
  const totalAssignmentPages = Math.ceil(filteredAssignments.length / ASSIGNMENTS_PER_PAGE) || 1;
  ...
  return (
    <main>
      ...
      <div className="page-body" style={{ maxWidth: "1160px", margin: "0 auto" }}>
        {statusMessage && (
        ...
        <div className="side-tabs-wrapper">
          ...
          <div className="side-tabs" style={{ display: isSidebarCollapsed ? "none" : "flex" }}>
            ...
            <button
              type="button"
              className={`tab-btn side-tab-btn ${adminActiveTab === "accounts" ? "active" : ""}`}
            ...
            <button
              type="button"
              className={`tab-btn side-tab-btn ${adminActiveTab === "assignments" ? "active" : ""}`}
            ...
            <button
              type="button"
              className={`tab-btn side-tab-btn ${adminActiveTab === "directory" ? "active" : ""}`}
              onClick={() => setAdminActiveTab("directory")}
            >
              Registered Accounts
            </button>
          </div>
          ...
          <div className="side-tab-content">
            ...
            {/* TAB 1: ACCOUNTS & PASSWORD MANAGEMENT */}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(360px, 1fr))", gap: "1.5rem", alignItems: "stretch" }}>
            ...
            <section className="card" style={{ height: "100%", display: "flex", flexDirection: "column" }}>
              ...
              <form onSubmit={handleCreateUser} style={{ flex: 1, display: "flex", flexDirection: "column", gap: "1rem" }}>
                <div>
                ...
                <button
                  ...
                  style={{ width: "100%", padding: "12px", cursor: "pointer", marginTop: "auto" }}
                >
            ...
            <section className="card" style={{ height: "100%", display: "flex", flexDirection: "column" }}>
              ...
              <p style={{ color: "#666", textAlign: "center", marginBottom: "1.25rem", fontSize: "0.88rem" }}>
                Students don't have login accounts and aren't listed here; system administrator passwords cannot be reset from this screen.
              </p>
              <form onSubmit={handleResetPassword} style={{ flex: 1, display: "flex", flexDirection: "column", gap: "1rem" }}>
                <div>
        ...
        {/* TAB 2: THERAPIST ASSIGNMENTS */}
        {adminActiveTab === "assignments" && (
          <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
            {/* ASSIGN THERAPIST TO STUDENT */}
            ...
            {/* ASSIGNED THERAPISTS & STUDENTS TABLE */}
            <section className="card">
                  ...
                  {filteredAssignments.length === 0 ? (
                    ...
                    paginatedAssignments.map((item, idx) => (
                      <tr key={`${item.therapist_id}-${item.student_id}-${idx}`} style={{ backgroundColor: idx % 2 === 0 ? "#ffffff" : "#f8faf5" }}>
                        <td style={{ padding: "12px 16px", borderRight: "1px solid #dbe5dc", borderBottom: idx === paginatedAssignments.length - 1 ? "none" : "1px solid #dbe5dc" }}>
                        ...
                        <td style={{ padding: "12px 16px", borderBottom: idx === paginatedAssignments.length - 1 ? "none" : "1px solid #dbe5dc" }}>
              ...
              {filteredAssignments.length > 0 && (
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "1rem", flexWrap: "wrap", gap: "0.5rem" }}>
                  <span style={{ fontSize: "0.88rem", color: "var(--ink-soft)" }}>
                    Showing {(assignmentsPage - 1) * ASSIGNMENTS_PER_PAGE + 1}–{Math.min(assignmentsPage * ASSIGNMENTS_PER_PAGE, filteredAssignments.length)} of {filteredAssignments.length} assignments
                  </span>
        ...
        {/* TAB 3: REGISTERED ACCOUNTS DIRECTORY */}
